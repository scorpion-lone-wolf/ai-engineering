...
**Version:** 1.3\
**Canonical repository file:** `docs/MASTER_CURRICULUM.md`\
...
## 0.1 Linear Progression Rule

This curriculum is taught in **one authoritative phase order: Phase 0 → Phase 24**.

- Do not start a later phase because its technology is exciting or immediately useful.
- If a later topic is mentioned early, explain only enough to answer the immediate question, record it in the parking lot when useful, and return to the current phase.
- If the current lesson exposes a missing prerequisite, temporarily repair that prerequisite and then return to the exact paused point.
- A phase is not complete until its required lessons, exercises, notes, graded assignments, project work, and mastery checks pass the completion gates defined later in this document.
- Reordering a topic is allowed only when intentionally correcting prerequisite order; the topic must remain in the curriculum.
...
The learning repository should maintain the following canonical Markdown files. Do not keep duplicate curriculum/status copies under alternate names such as `4_AI_ENGINEERING.md` once the canonical files exist.
...
## Cross-Cutting Depth Rule

Evaluation, observability, security, performance, deployment, and LLMOps appear as concerns in early projects because production engineers must notice them from the beginning. That does **not** permit teaching their later phases early.

Use this progression:

1. **Before the dedicated phase:** apply only the minimum **already-taught** baseline needed to keep the current project understandable and safe (for example, basic tests/logging, a simple task-success check, known-risk notes, or local packaging).
2. **At the dedicated phase:** teach the topic deeply and complete its required exercises/project work.
3. **After the dedicated phase:** revisit earlier major systems and harden them with the newly learned capability.

Therefore an early project may record an evaluation/security/observability requirement as pending without importing Phase 14/15/16 implementation prematurely.

## No-Jumping / Topic-Ownership Rule

Every nontrivial concept has one **owner phase** where it is first taught to working depth.

- An earlier phase must not require implementation of a concept whose owner phase has not yet occurred.
- A project that would benefit from a future concept records it as **deferred hardening** instead of teaching it early.
- A later phase may revisit an earlier concept only when it states the **new depth** being added (for example: basic tracing -> production observability; local workflow checkpoints -> distributed durable execution).
- A named framework, protocol, vendor feature, or current industry trend never overrides prerequisite order.
- When a topic appears in two phases without an explicit depth split, treat that as a curriculum defect and repair the ownership boundary before teaching it.
...
introduce prerequisites
    ->
teach the concept in its owner phase
    ->
practice it
    ->
integrate it with previously learned systems
    ->
optimize / harden it later
```

### Key Depth-Split Map

Use these ownership boundaries whenever the same concern reappears later:

- **Streaming:** Phase 1 = consume model token streams; Phase 19 = production async transport/backpressure/cancellation; Phase 23 = realtime multimodal/voice interaction.
- **Serving:** Phase 3 = architecture/model-selection literacy; Phase 18 = measurement, cache strategy, throughput/concurrency, and distributed serving optimization.
- **Evaluation:** early phases = simple local correctness/success checks only; Phase 14 = evaluation methodology, judge design, bias/leakage, regression systems, and CI gates.
- **Observability:** early phases = logs/debug traces only; Phase 15 = production tracing, telemetry semantics, privacy policy, dashboards, and alerting.
- **Security:** earlier protocol/project phases apply only already-taught safety constraints; Phase 16 owns general AI threat modeling and guardrail engineering; later new capabilities apply those learned controls without reordering Phase 16.
- **Model routing:** Phase 17 = gateway/routing architecture; Phase 18 = performance/cost optimization of routing; Phase 23 = advanced adaptive/reasoning-aware cascades.
- **Context:** Phase 13 = core memory/context policy; Phase 23 = advanced long-horizon context optimization.
- **Durability:** Phase 9 = workflow checkpoints/persistence; Phase 23 = distributed long-running replay/idempotency/recovery patterns.
...
**Depth boundary:** streaming here means consuming a model/provider token stream in one application process. Production WebSocket/SSE transport, cancellation, backpressure, and concurrent connection management are Phase 19; realtime multimodal/voice interaction is Phase 23. Fallback and rate limiting here are simple client-level safeguards; centralized gateway policy and routing are Phase 17.
...
**Reasoning-model boundary:** this phase teaches prompting and structured generation without depending on later inference-time-compute or adaptive-reasoning strategies. Reasoning-model routing, reasoning-effort control, and cost/quality trade-offs are taught in Phase 23 after the core production stack is understood.
...
- pipelines
- Transformers
- embedding-model family awareness only (embedding mechanics and retrieval are Phase 4)
- open-weight models
- quantization
- PEFT
- LoRA
- fine-tuning
- training-lifecycle literacy: train/validation/test split, batches, epochs, learning rate, loss, checkpoints, overfitting, and evaluation
- supervised fine-tuning (SFT)
- training-data curation and quality
- synthetic-data generation and filtering
- model distillation / teacher-student workflows
- preference optimization concepts (for example DPO-style methods)
- reinforcement fine-tuning / reward-based post-training concepts
- evaluation-driven decision: prompting vs model adaptation vs distillation; the full prompting-vs-RAG-vs-fine-tuning decision is revisited after Phase 5
- inference servers
- local models
...
- model licensing
- model selection
- architecture literacy for model selection & serving (GQA/MQA, KV cache, MoE, quantization interactions)
...
### Architecture Literacy for Model Selection & Serving

This is **engineering literacy, not architecture derivation** — no math proofs here (that belongs to a deep-learning theory track, if you're running one alongside this). The goal is: read a model's config, recognize what its architecture choices mean for cost/latency/memory, and pick/serve the right open-weight model for a constraint.

- reading a model's `config.json`/model card to identify its architecture family: `num_attention_heads` vs. `num_key_value_heads` (tells you MHA vs. MQA vs. grouped-query attention — GQA), `rope_theta`/position-embedding type, mixture-of-experts fields (`num_experts`, `num_experts_per_tok`)
- **why attention variant matters for serving:** fewer KV heads (MQA/GQA) means a smaller KV cache, which means more concurrent requests per GPU at the same memory budget — a direct cost/latency lever, not a theoretical detail
- **why MoE matters for serving:** large total parameter count (memory footprint to hold the model) vs. much smaller active-parameter count per token (compute cost) — different trade-off curve than a same-size dense model
- **KV cache as a first-class serving resource:** how context length and batch size trade off against it, KV-cache quantization as a separate lever from weight quantization
- weight quantization methods in practice (GPTQ, AWQ, bitsandbytes) and what each costs in accuracy/throughput
- modern inference engines and the serving techniques they're built around: vLLM, TensorRT-LLM, continuous batching, paged attention — connect each technique to the resource it's optimizing (KV cache memory, GPU utilization, request queuing)
- using this literacy to justify a model choice for a stated latency/cost budget, not just picking the "best benchmark" model

**Serving-depth boundary:** serving techniques in this phase are taught only to the depth needed to explain model-selection and memory/latency implications. Measurement, tuning, cache strategy, distributed serving, and production optimization are owned by Phase 18.

### Post-Training Engineering Literacy

The learner does not need to become a foundation-model researcher, but must understand the engineering decision boundary between changing **context** and changing **model behavior**.

Compare **only the mechanisms already taught at this point**:

```text
prompt / structured instructions
vs
SFT / behavior adaptation
vs
preference or reinforcement-based post-training
vs
distillation / smaller specialized model
```

The full decision matrix that includes **RAG / external knowledge** is deliberately revisited at the end of Phase 5, after retrieval and RAG have actually been learned.

Required judgment:

- use evaluation data before deciding to fine-tune;
- distinguish behavior-adaptation problems from missing/fresh-knowledge problems without implementing RAG early;
- understand catastrophic forgetting / capability regression at a conceptual level;
- understand synthetic-data contamination, low-quality teacher outputs, and distribution mismatch;
- measure quality, latency, and cost before and after adaptation.

**Evaluation-depth boundary:** adaptation evaluation here is limited to train/validation behavior, held-out task checks, and simple before/after comparisons. General LLM evaluation methodology, judge calibration, evaluator bias/leakage, online evaluation, and regression infrastructure are Phase 14.
...
- latency measurement
- memory measurement
- basic smoke/load measurement only; systematic performance optimization is Phase 18
- a short written justification of the chosen model's architecture (attention variant, dense vs. MoE, quantization) against this project's latency/memory constraints, using the Architecture Literacy section above
...
- reranking fundamentals (for example cross-encoder-style scoring)
...
- contextual retrieval fundamentals: preserve nearby/parent document context around a retrieved unit
- metadata
- hybrid search
- applying Phase 4 reranking inside a RAG pipeline
- query rewriting
- query expansion
...
### Adaptation Decision Revisit

Now that retrieval and RAG have been learned, revisit the Phase 3 model-adaptation decision with the full set of available mechanisms:

```text
prompting / structured instructions
vs
...
The learner must explain whether a problem is primarily about **knowledge**, **behavior**, **format**, **latency/cost**, or a combination before choosing an adaptation strategy.

**Advanced-retrieval boundary:** Phase 5 introduces basic parent/child and contextual retrieval inside a conventional RAG pipeline. Late interaction, structure-aware tree retrieval, retrieval ensembles/rank fusion, and reasoning-based retrieval are owned by Phase 6.
...
- reranking
- citations
- conversational history limited to an explicit short request/session window; durable memory policies are Phase 13
- evaluation
- observability
...
Move beyond a single "chunk -> embedding -> top-k" retrieval recipe and learn to choose retrieval architecture based on document structure, query type, scale, latency, explainability, and evidence requirements.
...
- structure-aware / hierarchical retrieval
...
The learner must compare retrieval families rather than treating vector search as the default answer to every knowledge problem.
...
hierarchical / parent-child retrieval
      |
structure-aware tree retrieval
      |
graph / relational retrieval
      |
reasoning-based retrieval
      |
direct long-context reading when retrieval is unnecessary
```

Required concepts:

- contextual retrieval: preserve document-level context around otherwise ambiguous chunks;
- rank fusion and retrieval ensembles;
- late-interaction / multi-vector retrieval concepts (ColBERT-style mental model);
- natural-section and layout-aware indexing instead of arbitrary fixed chunks;
- hierarchical/tree indexes for long professional documents;
- retrieval over tables, figures, sections, pages, and document structure;
- direct long-context prompting vs retrieval: when each wins on quality, latency, and cost;
- corpus-level routing followed by in-document retrieval;
- evidence provenance and page/section-level traceability;
- access-control-aware retrieval: authorization filters must survive indexing and retrieval;
- freshness, re-indexing, deletion, and source-of-truth synchronization.

**Phase boundary:** memory-aware retrieval is revisited in Phase 13 after memory is taught. Cache-aware retrieval is revisited in Phase 18 after caching is taught. They are not Advanced-RAG prerequisites.

**Security-depth boundary:** access-control-aware retrieval here means preserving already-known document/tenant visibility constraints as retrieval filters and provenance. Full authorization architecture, threat modeling, and adversarial security engineering remain Phase 16/19.

**Adaptive/corrective-RAG boundary:** corrective and adaptive RAG in this phase use bounded deterministic or conditional retrieval pipelines. They do not introduce an open-ended agent loop. Agentic RAG remains Phase 10.

**Graph boundary:** Phase 6 teaches graph/relational retrieval patterns and when graph evidence helps retrieval. Production knowledge-graph construction pipelines, entity/relation extraction at scale, graph data modeling/storage, and enterprise graph integration are advanced in Phase 23.

### PageIndex / Vectorless RAG Comparative Study

Use **PageIndex** as a current representative implementation of structure-aware, vectorless, reasoning-based retrieval — not as the definition of vectorless RAG.

Study:

- why semantic similarity is not identical to relevance;
- hierarchical document-tree construction;
- natural document sections vs artificial chunk boundaries;
- reasoning/search over the tree at query time;
- page/section traceability;
- single-document and multi-document/corpus routing;
- latency/cost implications of LLM-driven retrieval;
- failure modes when document structure is weak, noisy, or misleading.

#### Required experiment

Use one difficult long document such as an annual report, regulatory filing, legal contract set, or technical manual. Implement or configure:

1. dense-vector RAG baseline;
2. hybrid lexical + dense retrieval with reranking;
3. PageIndex-style structure-aware/vectorless retrieval;
4. direct long-context baseline when the document fits the model context.

Evaluate the same question set across all approaches using:

- retrieval recall / evidence hit rate;
- answer correctness / faithfulness;
- citation accuracy;
- table/figure question performance;
...
The output is an engineering decision record explaining **when each retrieval architecture should and should not be used**.

**Reasoning-retrieval boundary:** a structure-aware retriever may use one or more bounded model calls internally to score, navigate, or select from a fixed retrieval structure. It is still a retriever. It does **not** expose an open-ended tool-selection loop, persistent autonomous planning, arbitrary action execution, or self-directed retries. Those agent behaviors are reserved for Phase 10.

**Evaluation-depth boundary:** the comparison in this phase uses a fixed question set and simple retrieval/answer/citation measurements sufficient to compare retrieval architectures. Evaluator calibration, LLM-as-judge methodology, leakage/bias analysis, online evaluation, regression infrastructure, and CI evaluation gates remain Phase 14.

**Deferred prerequisite topic:** agentic RAG remains mandatory, but it is taught in **Phase 10 — Advanced Agents & Deep Agents** after the learner has completed agents from scratch and LangGraph/stateful workflows. Do not implement an agent merely to satisfy the Advanced RAG phase.
...
- document layout
- reading order
- page regions / bounding boxes
- table structure
- figure-to-caption relationships
- multimodal embeddings
- vision-language models
- text extraction vs page-image reasoning
- citation grounding back to page/region evidence

### Advanced RAG Failure Taxonomy

Be able to distinguish:

- parsing failure;
- chunk/section boundary failure;
- index freshness failure;
- candidate-retrieval failure;
- ranking/reranking failure;
- routing failure;
...
- hybrid retrieval
- reranking
- structure-aware/vectorless retrieval comparison
...
- streaming
- structured output
- basic message/state interfaces used by chains; long-term and agent-memory design is intentionally deferred to Phase 13
- callback/debug hooks that later support observability; production observability integrations are deferred to Phase 15

**Agent boundary:** do not teach or implement LangChain agent abstractions in this phase. The learner first builds the agent loop from scratch in Phase 8. LangChain agent abstractions are revisited comparatively in Phase 9, after the underlying control loop is understood.
...
# 14. Phase 8 --- Agent Fundamentals From Scratch
...
- durable execution
- debugging
- framework-native debugging / trace inspection only; LangSmith/Langfuse-style production observability is Phase 15
- LangChain agent abstractions, taught **now** as a comparison against the Phase 8 from-scratch loop (what the framework hides, gains, and constrains)
...
At this phase, planner/researcher/critic/fact-checker/writer may be implemented as **nodes/roles inside one stateful workflow**. Do not turn this project into the independent multi-agent coordination patterns reserved for Phase 11.
...
- long-running agents
- planning
- bounded working-context management for the current task (full memory/context engineering is Phase 13)
- scratchpads
- task decomposition
- dynamic tool selection
- bounded helper/sub-agent delegation under one parent runtime
- agent supervision
- agent interruption
- recovery
- agent reliability
- coding agents
- research agents
- agentic RAG, deferred from Phase 6 so retrieval is combined with agent planning/tool use only after agent and LangGraph foundations exist
- autonomous workflows

**Context boundary:** Phase 10 may manage only the information needed to execute the current task (scratchpads, tool results, temporary summaries, subtask context). Durable memory, retrieval from long-term memory, forgetting, context budgeting, and persistent context policy are owned by Phase 13.

**Multi-agent boundary:** helper/sub-agent delegation here remains parent-controlled and bounded: one orchestrator owns the goal, state, permissions, and stopping condition. Independent peer agents, shared-vs-isolated state design, negotiation/coordination, and multi-agent communication topologies are owned by Phase 11.
...
- planning
- parent-controlled bounded subtask/helper delegation
- tool use
- persistent state
- artifact generation
- verification
...
- shared state
- isolated state
- application-internal agent message contracts and coordination protocols; standardized cross-system agent interoperability is Phase 23
...
**Security boundary:** permissions in this project are simple explicit allowlists/role constraints already understood from tool execution and application design. Do not introduce AI threat modeling, guardrail systems, or a dedicated security-reviewer agent before Phase 16.
...
- schemas
- stateless vs stateful protocol behavior
- tasks / long-running operations
- extensions and version negotiation
- authentication
- OAuth/OIDC-oriented authorization patterns
- authorization
- permissions
- confused-deputy and token-forwarding risks
- server lifecycle
- error handling
- security

**Scope boundary:** authentication, authorization, permissions, security, logging, and rate limiting here are taught only to the depth required to build a safe MCP service. OAuth/OIDC, confused-deputy, and token-forwarding concerns are taught strictly as **MCP protocol-level authentication/authorization concerns**, not as general AI-security engineering. Full AI-security/guardrail engineering remains Phase 16. Production cloud deployment remains Phase 20.
...
7. Error handling
8. logging
9. local/service packaging and runtime validation; production cloud deployment is deferred to Phase 20
...
# 19. Phase 13 --- Agent Memory & Context Engineering
...
- entity memory
- reflection memory
- filesystem/tool-based memory for long-running agents
- memory routing
- forgetting
- decay
...
Context engineering is taught here as the policy for constructing the model's **working context**, not as a synonym for memory.

Study:

- context sources: system instructions, current user input, conversation window, retrieved evidence, tool results, scratchpads, and long-term memory;
- context budgeting and priority under finite context windows;
- selection, ordering, truncation, summarization, and compaction;
- separating durable memory from temporary working context;
- tool-result compression and provenance preservation;
- context isolation between subtasks/agents where needed;
- stale/conflicting context and source precedence;
- long-context failure patterns such as irrelevant-context dilution and lost-in-the-middle effects;
- deciding what should be omitted rather than continually appended.

**Advanced boundary:** Phase 13 owns the core context-policy model. Phase 23 later revisits long-horizon context optimization for durable, realtime, browser/computer-use, and multi-agent systems; it must not reteach basic context engineering.
...
- retrieval
- forgetting/decay
- basic memory behavior checks; rigorous memory/evaluator methodology is hardened in Phase 14
...
# 21. Phase 15 --- LLM Observability & Tracing
...
- dashboards
- alerting
- trace context propagation across model, retrieval, tool, MCP, and agent boundaries
- queue/worker trace propagation is revisited after queues/workers are introduced in Phase 19
- GenAI semantic conventions / standardized telemetry attributes
- privacy-aware prompt/completion capture policies
...
- vector/embedding-store access-control failures
- retrieval authorization bypass / cross-tenant leakage
- malicious MCP/tool servers and tool-supply-chain compromise
- untrusted peer-agent messages
...
**Future-capability rule:** browser/computer-use-specific attacks are not taught here because computer-use is introduced in Phase 23. When computer-use is learned there, the already-learned Phase 16 principles (untrusted content, least privilege, tool authorization, sandboxing, approval, audit) are applied immediately to that capability.
...
**Routing boundary:** this security project does not introduce multi-model routing. Phase 17 adds gateway routing/fallback/load-balancing after the security controls are understood, then carries those controls forward into the routed gateway.
...
- model selection
- policy routing
- cache integration points / cacheability policy awareness only; caching algorithms and implementation are Phase 18
- budget controls
...
- rate limits
- cost controls
- cache interface / deferred Phase 18 retrofit
- observability
- security policies

**Caching boundary:** Project 17 defines where a cache could sit and what must be part of a safe cache key, but does not implement semantic/prompt/prefix caching. Phase 18 owns cache behavior, measurement, invalidation, hit-rate analysis, and performance tuning, then retrofits Project 17.
...
- KV-cache memory as a capacity constraint
- caching
- semantic caching
- prompt/prefix caching
- KV-cache reuse
- speculative decoding concepts
- performance/cost-aware tuning of the Phase 17 routing policy
- smaller-model cascade routing based on measured quality/latency/cost
- context reduction
- retrieval optimization
- tensor/pipeline/data-parallel serving concepts where relevant
- disaggregated prefill/decode serving concepts
- KV-cache-aware routing
- capacity signals (TTFT, inter-token latency, queue depth, throughput) that later drive autoscaling in Phase 20
- embedding cost
- inference cost
- database cost

**Deployment boundary:** Phase 18 measures capacity and derives scaling thresholds. It does not configure production autoscaling infrastructure; cloud/Kubernetes autoscaling is owned by Phase 20.
...
**Streaming-depth boundary:** Phase 19 owns production text/event streaming transport (WebSockets/SSE), cancellation, concurrency, connection lifecycle, and backpressure. Voice activity detection, turn-taking, duplex audio, and realtime multimodal interaction remain Phase 23.
...
- reliability
- incident response
- operational governance for AI changes: ownership, approvals, version/audit evidence, release controls, and incident accountability

**Governance boundary:** broader organizational AI governance, policy frameworks, risk classification, and emerging regulatory/risk-management patterns are revisited in Phase 23.
...
- autonomy limits
- agent cost budgets
- recovery/monitoring of persisted jobs using mechanisms already learned through Phase 20; advanced replay-safe distributed durable-agent execution is Phase 23
...
11. Model adaptation / distillation pipeline

**Phase-22 design gate:** every design problem in this phase must use only capabilities already taught through Phase 21. Agent-to-agent interoperability standards, computer-use, and real-time multimodal/voice systems are Phase 23 topics and therefore must not appear as Phase 22 design requirements. They receive their own design exercises after they are taught in Phase 23.
...
These topics are deliberately placed after the core. Phase 23 **integrates and extends** earlier concepts; it must not silently reteach them under new names.

## Topics

- advanced long-horizon context engineering: dynamic compaction, context budgeting across long-running tasks, tool-result compression, and context isolation across delegated work (builds on Phase 13);
- reasoning-model / inference-time-compute strategies;
- adaptive reasoning effort and cost/quality trade-offs;
- advanced model cascades and task-aware routing that build on Phase 17 rather than reteaching gateway basics;
- advanced long-context strategies and retrieval/context compression that build on Phases 6 and 13;
- graph-assisted enterprise knowledge patterns that build on Phase 6 graph/relational retrieval: production knowledge-graph construction, entity/relation extraction, graph data modeling/storage, multi-hop evidence, and hybrid graph+lexical+dense retrieval;
- production SWE/coding-agent architecture that builds on the Phase 10 coding-agent foundation plus later security, evaluation, observability, and backend engineering;
- multimodal agents;
- browser agents and computer-use agents;
- sandboxed UI automation, permissions, action confirmation, and recovery;
- applying Phase 16 security to computer use: untrusted page/UI content, prompt injection, least privilege, sandboxing, approval, and audit;
- voice agents and real-time multimodal AI;
- interruption, turn-taking, voice activity detection (VAD), streaming audio, realtime tool use, cancellation, and latency budgets;
- advanced durable-agent execution beyond Phase 9 local checkpoints: cross-process resume, replay safety, idempotent side effects, leases/ownership, and long-running job recovery;
- asynchronous and event-driven agents built on Phase 19 queues/workers and Phase 20 deployment knowledge;
- agent interoperability and discovery;
- A2A-style agent-to-agent protocol concepts (agent cards, tasks, messages, artifacts, streaming);
- MCP vs agent-to-agent protocols: tools/resources integration vs peer-agent delegation;
- human-agent collaboration;
- organizational AI governance and AI risk management, building on Phase 21 operational governance.

### Required Phase-23 Design / Comparison Exercises

1. **Reasoning-effort benchmark:** run one task set across at least two reasoning-effort/model-routing strategies and compare quality, latency, and cost.
2. **Agent-interoperability design:** design or minimally implement two independently deployed agents that discover/delegate through an agent-to-agent contract; explain why MCP alone is or is not sufficient.
3. **Computer-use safety lab:** execute a bounded browser/UI task in a sandbox and trace permission boundaries, untrusted-content handling, approval points, and recovery.
4. **Realtime multimodal lab:** build or prototype a realtime session with streaming input/output, interruption/cancellation, latency measurement, and at least one tool call; add audio/VAD when the chosen model/runtime supports it.
...
### Project 22 / Extension 1: Enterprise Knowledge Engine
...
### Project 23 / Extension 2: Enterprise Workflow Automation Agent
...
### Project 24 / Extension 3: AI Coding / Software Engineering Agent
...
### Project 25 / Extension 4: Real-Time Streaming AI Service
...
- load testing
- stage 2 where supported: streaming audio input/output, interruption, VAD/turn-taking, and realtime tool use
...
The **numbered projects (`Project 0` through `Project 25`) and their repository folders are the canonical implementation sequence**.

The lettered `Project A` through `Project R` list below is a **capability-family / portfolio summary**, not a second execution order and not an additional set of unrelated projects. A numbered phase project may contribute to one or more lettered capability families. Do not build the lettered list separately from scratch.
...
# 31. Phase 24 --- Final Capstone: Enterprise AI Operating Platform
...
No project is complete merely because "the code works."

Engineering requirements are **progressively hardened** so this section cannot force the learner to jump ahead of the phase order.

For a project in the current phase:

- requirements whose concepts have already been taught must be implemented to the current curriculum standard;
- requirements that belong to a later dedicated phase must be recorded explicitly as **deferred hardening**, not silently implemented with unexplained future technology;
- after the dedicated phase is learned, earlier major systems should be revisited where appropriate and upgraded.

By the time a project is considered **portfolio/production complete**, where applicable it has:
...
This is not a second roadmap. It is a compact rendering of the authoritative phase order in Section 5.
...
Phase 5  RAG Engineering
  ↓
Phase 6  Advanced RAG (non-agentic foundations)
  ↓
Phase 7  LangChain Engineering (non-agentic abstractions)
  ↓
...
Phase 12 MCP
  ↓
Phase 13 Memory & Context Engineering
  ↓
Phase 14 LLM Evaluation
  ↓
Phase 15 LLM Observability & Tracing
...
Phase 22 AI System Design
  ↓
Phase 23 Advanced Industry Patterns
         + Projects 22–25 industry extensions
  ↓
Phase 24 Final Capstone
```

Do not reorder MCP and memory, skip the open-model phase, omit Phase 23, or jump directly from system design to the capstone.
...
- 15--20 portfolio-worthy consolidated projects/exercises selected from the larger set of incremental phase deliverables (the repository may contain more numbered project folders because several are learning milestones rather than separate resume projects)
- 5--8 major systems
...
- [ ] Understand SFT data curation
- [ ] Understand synthetic-data quality risks
- [ ] Understand model distillation
- [ ] Understand preference/reinforcement post-training at an engineering level
- [ ] After completing RAG, decide when to use prompting vs RAG vs fine-tuning vs distillation
...
- [ ] Compare vector, hybrid, late-interaction, structure-aware/vectorless, and long-context retrieval
- [ ] Build a PageIndex-style reasoning/tree retrieval experiment
- [ ] Ground tables/figures/pages with traceable citations
- [ ] Enforce access-control-aware retrieval
- [ ] Evaluate RAG
...
- [ ] Explain bounded sub-agent delegation vs true multi-agent coordination
- [ ] Design durable long-running agent execution with replay-safe/idempotent side effects
- [ ] Build or analyze a sandboxed browser/computer-use workflow
- [ ] Evaluate reasoning-effort/model-routing trade-offs
...
- [ ] Secure MCP
- [ ] Monitor MCP
- [ ] Understand MCP versioning, tasks/extensions, and modern authorization patterns
- [ ] Explain MCP vs agent-to-agent protocols
- [ ] Explain agent discovery, tasks/messages/artifacts, and peer-agent delegation at a protocol level
- [ ] Design a minimal cross-agent interoperability boundary

## Memory & Context Engineering

- [ ] Construct and budget working context from instructions, history, retrieval, tools, and memory
- [ ] Compress/order context while preserving provenance and priority
...
- [ ] Retrieval traces
- [ ] Agent traces
- [ ] OpenTelemetry / GenAI semantic-convention concepts
...
- [ ] Autoscaling
- [ ] Load testing
- [ ] Understand prefill vs decode performance
- [ ] Understand prefix/KV caching and KV-aware routing
- [ ] Understand disaggregated serving at architecture level
- [ ] Build realtime streaming/cancellation flows and explain VAD/turn-taking when audio is used
...
- [ ] LLMOps platform
- [ ] Computer-use / browser-agent safety architecture
...
21. LLMOps/AgentOps
22. AI system design
23. Advanced industry patterns + industry extension projects
24. Final capstone
...
Treat prerequisite order as a hard invariant.
- Do not implement or deeply teach a concept before its owner phase.
- If a current project would benefit from a future-phase capability, mark it as deferred hardening.
- When revisiting a concept later, state exactly what new depth is being added.
- Do not let a new framework, protocol, benchmark, or industry trend pull future material into the current phase.
...
For fast-moving AI technologies, teach in this order:
1. the stable engineering problem;
2. the underlying mechanism/protocol/architecture;
3. the trade-offs and failure modes;
4. one or more current representative implementations;
5. migration/interoperability considerations.

Do not make a vendor API, framework name, benchmark, or transient product feature the conceptual foundation. When a named technology becomes obsolete, preserve the underlying lesson and replace only the representative implementation.
...
Every major project is progressively hardened. Do not introduce a later
phase's technology early merely to satisfy this checklist. Before the dedicated
phase, record the item as pending/deferred; after that phase is learned, retrofit
important earlier systems where appropriate.

By portfolio/production completion, every major project must include where applicable:
- architecture
- implementation
- tests
- evaluation
...
Use it as the **master specification** and use the persistent repository state to resume.

At the beginning of each substantial session the mentor should read, in order:

```text
docs/MASTER_CURRICULUM.md (relevant rules/current phase)
        +
docs/PROGRESS_STATE.md
        +
docs/LEARNING_STATE.md
        +
relevant docs/DECISIONS.md entries
        +
relevant existing notes
        +
current repository code when implementation matters
        ↓
CURRENT LEARNING POSITION
        ↓
EXACT NEXT TEACHING STEP
...
The learner may add new information such as a new constraint, confusion, or changed goal, but should **not** be required to manually restate what is already recorded in persistent state.

`PROGRESS_STATE.md` should carry machine-readable-like fields where useful, for example:

```json
{
  "phase": "Phase 5 — RAG Engineering",
  "topic": "Hybrid Search",
  "mastery_level": 2,
  "notes_completed": true,
...
A new chat is a **resume event**, not a curriculum reset.
